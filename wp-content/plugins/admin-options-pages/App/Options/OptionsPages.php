...
class OptionsPages extends DB
{
    ...
    public function getAllOptionsPagesAndFields()
    {
        ...
        return $data->map(function ($item) {
            if (isset($item['menu'])) {
                // $menu = $item['menu'];
                // $pages = Collection::make($item['pages']);

                // $firstPageWithValidCapability = $pages->filter(function ($item) {
                //     return User::hasCapability($item['capability']);
                // })->first();

                // if ($menu['menu_slug'] !== $firstPageWithValidCapability['menu_slug']) {
                //     $menu['menu_slug'] = $firstPageWithValidCapability['menu_slug'];
                //     $menu['menu_slug_suffix'] = $firstPageWithValidCapability['subpage_slug_suffix'];

                //     $pages = $pages->map(function ($item) use ($menu) {
                //         $item['parent_slug'] = $menu['menu_slug'];
            ...
            if (!isset($_REQUEST['page']) && $_SERVER['PHP_SELF'] !== $prefix . '/wp-admin/options.php') {
                return null;
            }
            ...
            return $_REQUEST['page'] === $page['menu_slug'] || $_SERVER['PHP_SELF'] === $prefix . '/wp-admin/options.php';
        })->map(function ($page) {
